...
class SerialRelay(Node):
    def __init__(self):
        ...
        self.ik_sub = self.create_subscription(
            ArmIK, "/arm/control/ik", self.send_ik, 10
        )
    ...
    def send_ik(self, msg):
        ...
        input_raw = np.array(
            [-msg.movement_vector.x, msg.movement_vector.y, msg.movement_vector.z]
        ...
        tempMsg.data = (
            "Current Angles: "
            + str(math.degrees(self.arm.current_angles[2]))
            + ", "
            + str(math.degrees(self.arm.current_angles[4]))
            + ", "
            + str(math.degrees(self.arm.current_angles[6]))
        )
        ...
        if self.arm.perform_ik(target_position, self.get_logger()):
            ...
            self.get_logger().info(
                f"IK Angles: [{str(round(math.degrees(self.arm.ik_angles[2]), 2))}, {str(round(math.degrees(self.arm.ik_angles[4]), 2))}, {str(round(math.degrees(self.arm.ik_angles[6]), 2))}]"
            )
            ...
            command = (
                "can_relay_tovic,arm,32,"
                + f"{math.degrees(self.arm.ik_angles[0])*10},{math.degrees(self.arm.ik_angles[2])*10},{math.degrees(self.arm.ik_angles[4])*10},{math.degrees(self.arm.ik_angles[6])*10}"
                + "\n"
            )
            # self.send_cmd(command)
            ...
            self.get_logger().info(
                f"IK Angles: [{str(math.degrees(self.arm.ik_angles[2]))}, {str(math.degrees(self.arm.ik_angles[4]))}, {str(math.degrees(self.arm.ik_angles[6]))}]"
            )
            # tempMsg = String()
